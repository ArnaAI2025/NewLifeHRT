...
namespace NewLifeHRT.External.Clients
{
    public class LifeFileApiClient
    {
        ...
        public async Task<OrderResponse?> SendOrderAsync(object orderRequest, CancellationToken cancellationToken = default)
        {
            try
            {
                return await PostInternalAsync<OrderResponse>("/lfapi/v1/order", orderRequest, cancellationToken);
            }
