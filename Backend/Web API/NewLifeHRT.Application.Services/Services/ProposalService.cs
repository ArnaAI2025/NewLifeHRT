    ...
    public class ProposalService : IProposalService
    {
        ...
        public async Task<List<ProposalBulkResponseDto>> GetAllAsync(List<int>? ids, Guid? patientId)
        {
            try
            {
                ...
                if (ids?.Any() == true)
                {
                    var statusValues = ids.Select(id => (StatusEnum)id).ToList();
        ...
        public async Task<BulkOperationResponseDto> UpdateProposalAsync(Guid id, ProposalRequestDto dto, int userId)
        {
            ...
            try
            {
                ...
                existingProposal.Status = (StatusEnum)dto.Status;
        ...
        /// appending rejection reasons to the proposal description, and creating an order if approved.
        /// </summary>
        public async Task<CommonOperationResponseDto<Guid>> UpdateProposalStatusAsync(Guid proposalId, int status, string? description, int userId)
        {
            ...
            if ((StatusEnum)status == StatusEnum.Approved)
            {
            ...
            if (proposal.Status == StatusEnum.Approved)
            {
        ...
        /// <summary>
        /// created proposal same like order.
        /// </summary>
