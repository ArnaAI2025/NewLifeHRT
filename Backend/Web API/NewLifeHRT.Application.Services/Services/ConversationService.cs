...
namespace NewLifeHRT.Application.Services.Services
{
    public class ConversationService : IConversationService
    {
        ...
        /// <summary>
        /// Sends message and insert into messages.
        /// </summary>
        ...
        /// <summary>
        /// Processes incoming SMS by inserting into messages if conversation exists; 
        /// otherwise creates a conversation and inserts the message.
        /// Also handles saving text and media content and notifies connected clients.
        /// </summary>
        public async Task<string> ProcessIncomingSmsAsync(SmsRequestDto request)
        {
            try
            {
                // Initialize identifiers and createdBy string
                Guid? patientId = null;
                Guid? leadId = null;
                int? assignedUserId = null;
                string createdBy = null;

                // Try to find patient using mobile number
                ...
                {
                    // If no patient found, try to find lead by mobile number
                    var lead = await _leadService.GetLeadByMobileNumber(request.From);
                    if (lead != null)
                    {
                        // For known lead, assign relevant IDs and user
                        leadId = lead.Id;
                ...
                // Retrieve existing conversation or create new one for given identifiers and sender
                var conversation = await GetOrCreateConversationAsync(patientId, leadId, assignedUserId ?? 0, request.From, createdBy);

                // Construct message request DTO for storing the inbound message
                ...
                // If text body is present, save it as message content
                ...
                // Prepare collection of message content DTOs for sending to clients
                var messageContentsDto = new List<object>();

                // If media attachments exist, process and save each one
                if (request.NumMedia.HasValue && request.NumMedia.Value > 0)
                {
                    // Use patientId or leadId as folder path identifier for media storage
                    Guid idForPath = patientId != null ? (Guid)patientId : (leadId ?? Guid.Empty);

                    foreach (var mediaUrl in request.GetAllMediaUrls())
                    {
                        // Fetch the media bytes and content type from Twilio media URL
                        var (mediaBytes, contentType) = await _smsService.GetTwilioMediaAsync(mediaUrl);

                        // Convert AMR audio files to MP3 before saving
                        ...
                        // Generate unique filename with timestamp and appropriate extension
                        ...
                        // Construct blob storage path and upload media
                        ...
                        // Save media content record linked to the message
                        await _messageContentService.CreateMessageContentAsync(new MessageContentRequestDto
                        {
                            MessageId = messageResponse.Id,
                        ...
                        // Add media info to DTO list for client notification
