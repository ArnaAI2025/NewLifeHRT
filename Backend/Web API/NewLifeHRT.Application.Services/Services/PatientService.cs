...
namespace NewLifeHRT.Application.Services.Services
{
    public class PatientService : IPatientService
    {
        ...
        public async Task<CommonOperationResponseDto<Guid?>> CreateAsync(CreatePatientRequestDto request, int? userId)
        {
            if (!userId.HasValue)
            {
                ...
                {
                    Id = null,
                    Message = "User information is required to create a patient."
                };
            }

            var creatorUserId = userId.Value;
            ...
            try
            {
                ...
                {
                    var savedAddress = await _addressService.CreateAddressAsync(request.Address, creatorUserId);
                ...
                var patient = new Patient(
                    ...
                    gender: request.Gender.HasValue ? (GenderEnum)request.Gender.Value : null,
                ...
                if (addressId != null && result.Id != null)
                {
                    ...
                    await _shippingAddressService.CreateAsync(shippingAddressRequestDto, creatorUserId, addressId, true);
                ...
                if (request.PatientCreditCards?.Any() == true)
                    await _patientCreditCardService.CreateCardsAsync(request.PatientCreditCards, result.Id, creatorUserId);

                // Agendas
                if (request.AgendaId?.Any() == true)
                    await _patientAgendaService.AddAgendasAsync(request.AgendaId, result.Id, creatorUserId);
                // Handle profile image file upload

                if (request.ProfileImageFile != null)
                {
                    // 1. Upload file & create attachment metadata
                    var attachment = await _attachementService.UploadAsync(request.ProfileImageFile, request.ProfileImageFileCategoryId.Value, patient.Id, creatorUserId);
                    await _patientAttachementService.createAsync(patient.Id, attachment.Id, creatorUserId);
                }
    ...
    .ToList();
        ...
        {
            var activePatients = await _patientRepository.FindAsync(p => p.IsActive);
            return activePatients.ToPatientCounselorInfoDtoList();
        }
