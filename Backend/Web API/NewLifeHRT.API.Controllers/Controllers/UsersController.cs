...
namespace MultiTenantTest.Controllers
{
    ...
    public class UsersController : BaseApiController<UsersController>
    {
        ...
        public async Task<IActionResult> BulkDeactivate([FromBody] BulkOperationRequestDto<int> request)
        {
            try
            {
                var userId = GetUserId();
                ...
                var response = await _userService.BulkToggleUserStatusAsync(request.Ids,(int) userId, true);
                return Ok(response);
            }
            catch (Exception ex)
            {
                return BadRequest("An error occurred while deactivating users.");
            }
        }

        [HttpPatch("activate-bulk")]
        public async Task<IActionResult> BulkActivate([FromBody] BulkOperationRequestDto<int> request)
        {
            try
            {
                var userId = GetUserId();
                ...
                var response = await _userService.BulkToggleUserStatusAsync(request.Ids,(int) userId, false);
                return Ok(response);
            }
            catch (Exception ex)
            {
                return BadRequest("An error occurred while activating users.");
            }
        }
