    ...
    public abstract class MultiTenantJobBase<TJob> where TJob: class
    {
        ...
        public async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var tenant in tenants)
            {
                ...
                TenantContextHolder.CurrentTenant = tenant.Domain;
                Logger.LogInformation("Starting Hangfire job {JobName} for tenant {Tenant}", typeof(TJob).Name, tenant.Domain);

                try
                {
                    await ExecuteForTenantAsync(cancellationToken);
                    Logger.LogInformation("Completed Hangfire job {JobName} for tenant {Tenant}", typeof(TJob).Name, tenant.Domain);
                }
                catch (Exception ex)
                {
                    Logger.LogError(ex, "Unhandled exception in Hangfire job {JobName} for tenant {Tenant}", typeof(TJob).Name, tenant.Domain);
                    throw;
                }
            }
        }
