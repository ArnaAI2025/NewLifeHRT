...
export class SidebarComponent {
  ...
  ngOnInit() {
    this.isPatientUser = this.appSettingService.isUserPatient();

    if (this.isPatientUser) {
      const patientId = this.appSettingService.getPatientUserId();
      this.addPatientEditLabel(patientId);
      this.filterNavItemsForPatients();
    }
    else {
      this.hidePatientEditLabel();
    }
  }
  ...
  filterNavItemsForPatients() {
    ...
    this.navItems = this.navItems.filter(item => !hiddenLabels.includes(item.label));
  }

  addPatientEditLabel(patientId: string) {
    const exists = this.navItems.some(item => item.label === 'My Profile');
    if (!exists) {
      this.navItems.push({
        label: 'My Profile',
        icon: 'person',
        route: `/patient/edit/${patientId}`
      });
    }
  }
  hidePatientEditLabel() {
    this.navItems = this.navItems.filter(item => item.label !== 'My Profile');
  }
